...
const Index = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (filterSegmento) result = result.filter(o => o.segmento === filterSegmento);
    return result;
  }, [orders, selectedAno, search, filterProduto, filterMotivo, filterSegmento]);
  ...
  const handleAdd = async (order: DelayedOrder) => {
    ...
    if (import.meta.env.VITE_SUPABASE_URL) {
      try { await supabase.from("delayed_orders").insert([order]); } catch { /* silently ignore */ }
    }
  };
  ...
  const handleEdit = async (order: DelayedOrder) => {
    if (editingOrder === null) return;
    setOrders((prev) => prev.map((o, i) => (i === editingOrder.index ? order : o)));
    if (import.meta.env.VITE_SUPABASE_URL) {
      try {
        await supabase.from("delayed_orders")
          .update(order)
          .eq("cliente", editingOrder.order.cliente)
          .eq("dataPrevista", editingOrder.order.dataPrevista);
  ...
  const handleDelete = async (index: number) => {
    if (!window.confirm("Confirmar exclusão deste pedido?")) return;
    const order = orders[index];
    setOrders((prev) => prev.filter((_, i) => i !== index));
    if (import.meta.env.VITE_SUPABASE_URL && order) {
      try {
        await supabase.from("delayed_orders")
          .delete()
          .eq("cliente", order.cliente)
          .eq("dataPrevista", order.dataPrevista);
      } catch { /* silently ignore */ }
    }
  };
